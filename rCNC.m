...
- (NSArray*)XSegmentKoordinatenMitRadiusA:(float)RadiusA mitRadiusB:(float)RadiusB mitWinkel:(float)Winkel mitLage:(int)Lage mitAnzahlPunkten:(int)anzahlPunkte
{
...
*/
   NSPoint StartpunktA = NSMakePoint(0, 0);
   NSPoint MittelpunktA = NSMakePoint(0, 0);

   NSPoint StartpunktB = NSMakePoint(0, 0);
   NSPoint MittelpunktB = NSMakePoint(0, 0);
   ...
   int index;
   for(index=0;index<anzahlPunkte;index++)
   {
      float tempAX=0;
      float tempAY=0;
      ...
      switch (Lage)
      {
         case 0: // nach rechts oben ueber Startpunkt
            ...
            tempAY=RadiusA*(cos(phi));
            break;
         case 1: // nach links oben von Startpunkt
            tempAX=RadiusA*(cos(phi))*-1;
            tempAY=RadiusA*sin(phi);
            break;
            
         case 2:
            tempAX=RadiusA*sin(phi)*-1;
            tempAY=RadiusA*(cos(phi))*-1;
            break;

         case 3:  // nach links unten von Startpunkt
            tempAX=RadiusA*(cos(phi));
      ...
      tempAX += MittelpunktA.x;
      tempAY += MittelpunktA.y;
